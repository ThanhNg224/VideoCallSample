...
public class OutgoingCallActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void makeCall() {
        ...
        //make a call
        StringeeClient client = ((MyApp) getApplication()).getClient();
        if (client == null) {
            Log.e("CALL_DEBUG", "❌ client is null. Cannot make call");
            finish(); // or show a toast and back out
            return;
        }

        stringeeCall = new StringeeCall2(client, from, to);  // ✅ SAFE
        ...
        stringeeCall.setCallListener(new StringeeCallListener() {
            ...
            public void onSignalingStateChange(StringeeCall2 stringeeCall2, final SignalingState signalingState, String reason, int sipCode, String sipReason) {
                runOnUiThread(() -> {
                    ...
                    Log.d("CALL_DEBUG", "📶 Signaling state: " + signalingState.name() + " | reason: " + reason + " | sip: " + sipCode + " - " + sipReason);
        ...
        stringeeCall.makeCall(new StatusListener() {
            ...
            public void onSuccess() {
                Log.d("CALL_DEBUG", "✅ makeCall success");
            }

            @Override
            public void onError(StringeeError error) {
                Log.e("CALL_DEBUG", "❌ makeCall error: " + error.getMessage());
            }
        });
    ...
    private void endCall() {
        stringeeCall.hangup(new StatusListener() {
            @Override
            public void onSuccess() {
                Log.d("CALL_DEBUG", "📞 Call initiated successfully");
